...
public class Ch00info {
	...
	// 기능(Function) : 객체마다 수행할수 있는 공통 Action(기능-메서드)
	...
	// 클래스로 객체를 생성하기 전엔느 기본적으로 메모리공간을 차지하지 않는다
	// (Document화)
	...
	// 1) 스택영역 : {}내에 생성되고 소멸되는영역(int,double...)
	...
	// Scanner sc : Scanner라는 크래스 자료형으로 정의된 참조변수 sc
