...
public class UserController implements SubController {
	...
	public Map<String, Object> execute(Map<String, Object> params) {
		...
		case 1: // C - 회원가입
			...
			response.put("message", "회원가입성공");
			}
			break;
		case 2: // R - 개인 회원 정보 조회
			System.out.println("[SC] 회원 정보 조회 요청 확인");
			// 01 파라미터 받기
			// 02 유효성검증(Data Validation)
			// 03 관련 서비스 실행
			// 04 뷰로 이동(or 내용전달)
		...
		case 3: // U - 개인 회원 정보 수정
			System.out.println("[SC] 회원 정보 수정 요청 확인");
			// 01 파라미터 받기
			// 02 유효성검증(Data Validation)
			// 03 관련 서비스 실행
			// 04 뷰로 이동(or 내용전달)
	...
	private boolean isValid(UserDto userDto) {
		...
		if (userDto.getUserid().matches("^[0-9].*")) {
			...
			response.put("error", "[INVALID] userid의 첫문자로 숫자가 들어올수 없습니다");
			return false;
		}
		// NULL 체크 / 데이터(자료) 수준에서의 의미있는데이터가 포함되어져있는지 여부
		// userid 은 첫문자가 숫자인지 여부 - /or 길이가 1글자인지 등등
		// username 은 첫 문자가 숫자인지 여부 -
		// password 복잡도 체크는 Business Layer 체크(Policy 에 의한 처리)
		return true;
	}
